...
run_one "fuse-none" "$TMP_FVSOL" "L4/tol0.1 sc OFF (baseline; unaffected by the port)"
build true; export NEON_MGSC_MODE=post
run_one "fuse-post" "$TMP_FVSOL" "L4/tol0.1 sc POST only, FUSED dots + faithful inner (was post 9.4 iters, p_ms 810)"
build true; export NEON_MGSC_MODE=both
run_one "fuse-both" "$TMP_FVSOL" "L4/tol0.1 sc BOTH, FUSED dots + faithful inner (was both 7.6-8.6 iters)"
