...
- Add in-situ post-processing (`system/postProcess.yaml` / `postProcess.py` tables written to `postProcessing/<name>.csv`), `setFields` region initialisation (`system/setFields.yaml` / `setFields.py`) and the `post_catalog`/`save_post`/`save_preprocess` MCP tools [#406](https://github.com/exasim-project/NeoFOAM/pull/406)
