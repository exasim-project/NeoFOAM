...
- Add the `incompressibleVoFNeoN` solver: interFoam-style VoF on the NeoN backend (SolverSpec/ModelSpec) — explicit MULES and semi-implicit MULESCorr phase advection with vanLeer + `cAlpha` interface compression, buoyant `p_rgh` PISO with gravity + surface tension via the model-owned `interfaceForce` extension point, and the supporting OpenFOAM-parity C++ kernels (`vofOperators`, two-phase mixture properties, `constrainPressure`, `updateVelocityBuoyant`); alpha kernels machine-precision-verified against OpenFOAM and the solver e2e-compared on damBreak against both the pybFoam `incompressibleVoF` solver and native `interFoam` [#356](https://github.com/exasim-project/NeoFOAM/pull/356)
