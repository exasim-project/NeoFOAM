...
GaussGreenDivKernel::GaussGreenDivKernel(const unstructuredMesh& mesh, const scalarField& phi, vectorField& gradPhi)
    : mesh_(mesh), phi_(phi), gradPhi_(gradPhi)
{
    NeoFOAM::fill(gradPhi_, NeoFOAM::Vector(0.0, 0.0, 0.0));
};

void GaussGreenDivKernel::operator()(const GPUExecutor& exec)
{
...
void GaussGreenDivKernel::operator()(const OMPExecutor& exec)
{
...
void GaussGreenDivKernel::operator()(const CPUExecutor& exec)
{
