...
// surfaceTensionForce reuses the identical nHatf (single source of truth).
fvcc::SurfaceField<NeoN::scalar> computeNHatf(const fvcc::VolumeField<NeoN::scalar>& alpha1)
{
    ...
    fvcc::SurfaceInterpolation<NeoN::Vec3> interp(
        exec,
        mesh,
        NeoN::Input {NeoN::TokenList {std::string("linear")}}
    );
    ...
    double sumV = 0.0;
    for (NeoN::localIdx c = 0; c < nCells; ++c)
        sumV += vView[c];
...
//   w_f       = limiter*wCD + (1 - limiter)*pos0(phi_f) (limitedSurfaceInterpolationScheme) wCD =
//   SfdNei/(SfdOwn + SfdNei)                          (linear/CD weight, makeWeights) limiter   =
//   (r + |r|)/(1 + |r|)                              (vanLeerLimiter) r         = NVDTVD::r(phi_f,
//   alpha_own, alpha_nei, gradc_own, gradc_nei, d)  (see below) gradc     = fvc::grad(alpha) (Gauss
//   linear == GaussGreenGrad),  d = C_nei - C_own
...
)
{
    ...
    auto wN = constructFrom(rt.exec, rt.nfMesh, rt.mesh.weights());
    ...
    );
    {
        ...
        forAll(dIf, f)
            dIf[f] = CC[fNei[f]] - CC[fOwn[f]];
    }
    auto dN = constructFrom(rt.exec, rt.nfMesh, dField);
    ...
    NeoN::parallelFor(
        ...
        KOKKOS_LAMBDA(const NeoN::localIdx f) {
            ...
            {
                const NeoN::scalar sgc = (gradcf > 0.0) ? 1.0 : ((gradcf < 0.0) ? -1.0 : 0.0);
                const NeoN::scalar sgf = (gradf > 0.0) ? 1.0 : ((gradf < 0.0) ? -1.0 : 0.0);
...
// with alpha2 = 1 - alpha1 and linearInterp affine, linearInterp(alpha2) = 1 -
// linearInterp(alpha1). Faithful to VoF/alphaEqn.H (icAlpha == scAlpha == 0, the damBreak /
// interFoam default).
...
// Foam::MULES::limiter inner sweep (MULESTemplates.C). Shared by mulesExplicitSolve and
// mulesCorrect: given the per-cell bound RHS (psiMaxn, psiMinn) and the antidiffusive-flux
// extrema sums (sumPhip, mSumPhim), iterate nLimiterIter times to compute the per-internal-face
// limiter lambda in [0, 1] for the correction flux phiCorr. Boundary lambda stays 1. Kernel-name
// strings are profiling labels only and do not affect the numerics — the loop order and
// arithmetic are identical to the two former in-line copies (bitwise parity preserved).
    ...
    for (int j = 0; j < nLimiterIter; ++j)
    {
        ...
        NeoN::parallelFor(
            ...
            KOKKOS_LAMBDA(const localIdx c) {
                sumlPhip[c] = Kokkos::max(
                    Kokkos::min((sumlPhip[c] + psiMaxn[c]) / (mSumPhim[c] + ROOTVSMALL), 1.0),
                    0.0
                );
                mSumlPhim[c] = Kokkos::max(
                    Kokkos::min((mSumlPhim[c] + psiMinn[c]) / (sumPhip[c] + ROOTVSMALL), 1.0),
                    0.0
                );
            },
            "mulesFCT::lambdaCells"
        );
        ...
        NeoN::parallelFor(
            ...
            KOKKOS_LAMBDA(const localIdx f) {
                if (phiCorr[f] > 0.0)
                    lambda[f] =
                        Kokkos::min(lambda[f], Kokkos::min(lambdap[own[f]], lambdam[nei[f]]));
                else
                    lambda[f] =
                        Kokkos::min(lambda[f], Kokkos::min(lambdam[own[f]], lambdap[nei[f]]));
            },
            "mulesFCT::lambdaFaces"
        );
    }
    return Lambda;
}
...
// mesh views / Vector); its sibling mulesCorrect lives just below.
...
)
{
    ...
    // FCT limiter sweeps (shared kernel).
    NeoN::Vector<scalar> Lambda =
        computeFCTLambda(exec, mesh, phiCorr, psiMaxn, psiMinn, sumPhip, mSumPhim, nLimiterIter);
    auto lambda = Lambda.view();
...
)
{
    ...
    NeoN::parallelFor(
        ...
        KOKKOS_LAMBDA(const localIdx bf) {
            ...
            if (c > 0.0) Kokkos::atomic_add(&sumPhip[bOwn[bf]], c);
                ...
                Kokkos::atomic_sub(&mSumPhim[bOwn[bf]], c);
        },
    ...
    NeoN::Vector<scalar> Lambda =
        computeFCTLambda(exec, mesh, pc, psiMaxn, psiMinn, sumPhip, mSumPhim, nLimiterIter);
    auto lambda = Lambda.view();
    ...
    fvcc::surfaceIntegrate<NeoN::scalar>(
        ...
        own,
        bOwn,
        phiCorr.internalVector().view(),
        phiCorr.boundaryData().value().view(),
        V,
        Div.view(),
        NeoN::dsl::Coeff(1.0)
    );
...
// on an orthogonal mesh); on a non-orthogonal mesh the missing non-orthogonal correction
// makes this silently wrong.
fvcc::SurfaceField<NeoN::scalar>
surfaceTensionForce(const fvcc::VolumeField<NeoN::scalar>& alpha1, double sigma)
{
    const auto exec = alpha1.exec();
    const auto& mesh = alpha1.mesh();
