...
std::unordered_map<std::string, CompressionMethodFactory::TCreateMethod>
    CompressionMethodFactory::s_methods;

bool CompressionMethodFactory::Register(
    const std::string name, CompressionMethodFactory::TCreateMethod funcCreate
)
{
...
)
{
    ...
    else
    {
        ...
        return surfaceInterpolation(
            exec, mesh, std::make_unique<upwind>(exec, mesh)
        ); // for the compiler
    }
}
