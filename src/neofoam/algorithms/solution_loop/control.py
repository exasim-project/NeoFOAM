...
class SolutionControl(BaseModel):
    """Outer solution-loop control — advance time only.

    Convergence/stopping is no longer a SolutionControl concern: residual
    convergence and every other stop criterion flow through the ``loopCondition``
    fold (``set_time_step`` records ``keep_running``; ``SolutionLoopPredicate`` ANDs
    it with ``running()``). This control just governs advancement, mirroring
    ``Foam::Time`` stepping.
    """
    ...
    def run(self, loop: Any) -> bool:
        """Outer-loop predicate: report whether more steps remain (advance only)."""
