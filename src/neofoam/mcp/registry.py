...
Importing a solver spec pulls ``pybFoam`` (the native bindings), so the registry
must not import specs at module-import time. Discovery is **filesystem-only**
(no spec import): a solver is any package ``neofoam.solver.<name>`` carrying a
module ``<name>.py`` that exports a ``<name>`` spec object — the convention every
solver package follows (e.g. ``incompressibleFluid/incompressibleFluid.py`` →
``incompressibleFluid``). New solvers register automatically by following that
layout; there is no per-solver line to add here. ``resolve_solver`` imports the
spec lazily (pulling ``pybFoam``) only when actually called.
"""
