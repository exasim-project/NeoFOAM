...
BUILD   → emit lazy InitSteps that construct the ``blockamr`` mesh and inject the
          blockAMR loop/writer backends through the framework seams. ``U`` / ``p``
          / ``phi`` and the values their equations need are built by the
          projection model's own ``@build`` (which owns its configs).
...
block-structured DSL (``blockamr``) carries the physics. The mesh and the mesh
dict are init-only resources (the leading underscore keeps them off the Context)
shared with the projection model, which reads them by name.
"""
...
from .configs import ControlDictConfig, MeshDictConfig
...
def create_init(case_dir: Optional[Path] = None) -> StagedInitRunner:
    ...
    def build_lazy(
        ...
        # --- init-only resources: the mesh dict + the mesh it builds. The
        # projection model reads both by name (it derives the velocity/pressure
        # BCs from the mesh dict and creates its fields on the mesh). ---
        ...
        # (PlotfileWriteHook, logger, reporter) are injected by the
        # *_backend_steps through the framework seams.
