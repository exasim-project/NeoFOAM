...
"""courant — the CFL deltaT limit as a timeStepConstraint contribution (NeoN VoF).
...
CFL-adjust half (the ``SolutionLoop`` growth cap of 1.2 matches interFoam's
``setDeltaT.H``).

The model registers with the NeoN VoF family (so it is discoverable case-free)
and the contribution is bound to the model via
...
First-version note: the interface (alpha) Courant limit ``maxAlphaCo`` is not
yet enforced — on damBreak the interface Courant is bounded by the flow Courant,
so the flow CFL limit is sufficient (see the incompressibleVoFNeoN spec).
"""
