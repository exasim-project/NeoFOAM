...
"""NeoN backend wiring for the framework ``fieldWriter`` core Model (VoF).
...
write (``Time.write(True)``) cannot see them. :class:`NeoNWriteHook` writes the
flagged fields (``alpha1`` / ``p_rgh`` scalars + ``U`` vector) through
``nfb.write_scalar_field`` / ``write_vector_field``, exactly like the legacy
``neoInterFoam`` ``outputTime()`` branch. Laminar — no turbulence fields.
"""
...
class NeoNWriteHook(FieldHook):
    """Persist NeoN VoF fields via the NeoN writers (a :class:`FieldHook`).

    Receives only the fields flagged ``write=True`` (``alpha1`` / ``p_rgh`` /
    ``U``), dispatched by their Context registration name to the scalar/vector
    writer. The ``NeoNTimeSync`` backend has already synced ``Foam::Time``, so
    everything lands in the right time directory.
    """

    field_hook_type: Literal["neonVoF"] = "neonVoF"
    runtime: Any = None  # the NeoN RunTime adapter
    scalar_names: tuple[str, ...] = ("alpha1", "p_rgh")
...
def neon_writer_backend_steps() -> list[InitStep]:
    ...
    * registers ``step_reporter`` — the per-step timing report the framework
    ...
    def inject_hook(ctx: dict[str, Any]) -> FieldHook:
        hook = NeoNWriteHook(runtime=ctx["_neon_runtime"])
    ...
    return [
        model(
            "writer_hook",
            inject_hook,
            depends_on=["models.writer", "_neon_runtime"],
        ),
