...
This solver reimplements the C++ neoIcoFoam solver entirely in Python,
using the following building blocks:
  - _neon:             NeoN core (fields, DSL operators, mesh, linear algebra)
  - neofoam_bindings:  NeoFOAM adapter (Runtime, PisoControl, PDESolver, PISO helpers)
"""
...
class NeoIcoFoam:
    ...
    def run(self) -> None:
        ...
        while rt.loop():
            ...
            # Store old time U (needed for ddt operator)
            old_U = nn.old_time(U)
            old_U.assign(U)
