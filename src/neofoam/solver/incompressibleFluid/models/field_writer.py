...
build a :class:`~neofoam.algorithms.writer.FieldWriter`, and the per-step
...
* the two :class:`~neofoam.algorithms.writer.FieldHook` backends
...
from neofoam.algorithms.field_writer import (  # re-exported for the solver
    build,
    fieldWriter,
    write_output,
)
from neofoam.algorithms.writer import FieldHook
...
class RuntimeWriteHook(FieldHook):
    ...
    current (stepper-synced) time, including sub-model fields not present in
    ``ctx.fields``. The ``fields`` argument is ignored (the registry knows them).
    The live ``runtime`` is held in an ``Any`` field (injected at build, not from
    the file).
    """
...
def make_field_hook(config: ControlDictConfig, runtime: Any) -> FieldHook:
    """Select the write backend from ``config.writeBackend``."""
    if config.writeBackend == "perField":
        return PerFieldWriteHook()
    return RuntimeWriteHook(runtime=runtime)
...
def writer_backend_steps(config: ControlDictConfig) -> list[InitStep]:
    ...
    * registers ``step_reporter`` — the per-step timing report the framework
    ...
    def inject_hook(ctx: dict[str, Any]) -> FieldHook:
        hook = make_field_hook(config, ctx["runtime"])
        ctx["models.writer"].hook = hook
        return hook

    def make_reporter(ctx: dict[str, Any]) -> Any:
        runtime = ctx["runtime"]
        return lambda: runtime.printExecutionTime()

    return [
        model("writer_hook", inject_hook, depends_on=["models.writer", "runtime"]),
        model("step_reporter", make_reporter, depends_on=["runtime"]),
    ]
