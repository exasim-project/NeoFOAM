...
before the solver opens any C++ runtime. ``constant/transportProperties`` and
``constant/g`` are read directly by the NeoN C++ factories
(``read_two_phase_transport_properties`` / ``read_gravity`` / ``create_gh``),
so no config class is declared for them here.
"""
