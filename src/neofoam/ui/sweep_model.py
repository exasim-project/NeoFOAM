...
from neofoam.tooling.workflow.rules import DEFAULT_ENABLED, MESH_DIM
from neofoam.tooling.workflow.sweep import cross_product, variant_errors
...
def series_values(mode: str, values_text: str, vmin: str, vmax: str, count: str) -> list[float]:
...
class SweepModel:
    ...
    def add_cad_dimension(
        ...
        *,
        ...
        schema: dict[str, Any] = {
            "type": "object",
            "properties": {alias: {"type": "number", "title": alias} for alias in params},
        }
        self.dims[name] = DimensionState(
            name=name,
            title=title,
    ...
    def remove_mesh_source(self, config_name: str) -> None:
        ...
        if mesh is None or config_name not in mesh.schema.get("properties", {}):
            ...
            raise ValueError(msg)
        props = {k: v for k, v in mesh.schema.get("properties", {}).items() if k != config_name}
        if not props:
    ...
    def generate_series(self, name: str, param: str, values: list[float], *, replace: bool) -> int:
    ...
    def variant_error(self, name: str, classes: dict[str, Any]) -> str:
        ...
        if dim is None or dim.kind == "cad":
            return ""
        errors = variant_errors({name: {dim.selected: dim.entries[dim.selected]}}, classes)
