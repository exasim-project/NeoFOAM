...
/* @brief given an unstructured mesh this function computes row and column indices in CSR format
 *
 * @return a tuple containing rowPtrs, colIdxs, and a faceOrderMap
 * the faceOrderMap is a Field of length = 2*internalFaces
 * where faceOrderMap[2*faceI]     = globalIdx of owner
 * and   faceOrderMap[2*faceI + 1] = globalIdx of neighbour
 * with  globalIdx being the consecutive index in the resulting CSR/COO matrix
 */
template<typename IndexType>
std::tuple<Field<IndexType>, Field<IndexType>, Field<IndexType>>
createSparsityPattern(const Executor& exec, const UnstructuredMesh& in)
{
    const auto nInternalFaces = in.nInternalFaces();
    const auto nCells = in.nCells();
    const auto nnzs = 2 * nInternalFaces + nCells;
    const auto [faceOwner, faceNeighbour] = spans(in.faceOwner(), in.faceNeighbour());

    Field<IndexType> rows(exec, nnzs, 0);
    Field<IndexType> cols(exec, nnzs, 0);

    auto [rowss, colss] = spans(rows, cols);

    parallelFor(
        exec,
        {0, nInternalFaces},
        KOKKOS_LAMBDA(const size_t facei) {
            IndexType owner = static_cast<IndexType>(faceOwner[facei]);
            IndexType neigh = static_cast<IndexType>(faceNeighbour[facei]);
            rowss[2 * facei] = owner;
            rowss[2 * facei + 1] = neigh;
            colss[2 * facei] = neigh;
            colss[2 * facei + 1] = owner;
        }
    );
    ...
    parallelFor(
        ...
        KOKKOS_LAMBDA(const size_t cellI) {
            rowss[2 * nInternalFaces + cellI] = cellI;
            colss[2 * nInternalFaces + cellI] = cellI;
        }
    );


    // TODO: NOTE: As a proof of concept things are sorted on the CPU for now
    // until a GPU sorting is implemented

    auto [rowsH, colsH] = copyToHosts(rows, cols);
    auto [rowsHS, colsHS] = spans(rowsH, colsH);

    std::vector<IndexType> permutation(rowsH.size());
    std::iota(permutation.begin(), permutation.end(), 0);
    std::stable_sort(
        permutation.begin(),
        permutation.end(),
        [&](std::size_t i, std::size_t j)
        { return std::tie(rowsHS[i], colsHS[i]) < std::tie(rowsHS[j], colsHS[j]); }
    );

    auto sortedRows = detail::apply_permutation(rowsHS, permutation);
    auto sortedCols = detail::apply_permutation(colsHS, permutation);

    return {
        Field<IndexType> {exec, sortedRows.data(), sortedRows.size()},
        Field<IndexType> {exec, sortedCols.data(), sortedCols.size()},
        Field<IndexType> {exec, permutation.data(), permutation.size()}
    };
}
