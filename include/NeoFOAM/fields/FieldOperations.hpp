...
**
*/
template <typename T> struct ApplyOp {
...
template <typename T, typename Func> void map(Field<T> &f, Func func) {
  ApplyOp<T> func_;
  std::visit([&](const auto &exec) { func_(exec, f, func); }, f.exec());
}

template <typename T> struct fillOp {
  T fillValue_;
...
template <typename T> void fill(Field<T> &f, const T value) {
  fillOp<T> fill_(value);
  std::visit([&](const auto &exec) { fill_(exec, f); }, f.exec());
}

template <typename T> struct setFieldOp {
...
template <typename T> void setField(Field<T> &f, const Field<T> &rhsField_) {
  if (f.size() != rhsField_.size()) {
    f.setSize(rhsField_.size());
  }
  setFieldOp<T> setField_(rhsField_);
  std::visit([&](const auto &exec) { setField_(exec, f); }, f.exec());
}
...
template <typename T> void add(Field<T> &f, const Field<T> &rhsField) {
  addOp<T> addField_;
  std::visit([&](const auto &exec) { addField_(exec, f, rhsField); }, f.exec());
}
...
template <typename T> void sub(Field<T> &f, const Field<T> &rhsField) {
  subOp<T> subField_;
  std::visit([&](const auto &exec) { subField_(exec, f, rhsField); }, f.exec());
}
...
template <typename T> void mul(Field<T> &f, const Field<T> &rhsField) {
  mulOp<T> mulField_;
  std::visit([&](const auto &exec) { mulField_(exec, f, rhsField); }, f.exec());
}

template <typename T> void mul(Field<T> &f, T s) {
  mulOp<T> mulField_;
  std::visit([&](const auto &exec) { mulField_(exec, f, s); }, f.exec());
}
