...
from foamadapter.framework.operations import Operation, OperationCollection, SequentialOp, StepBuilder, IterativeOp, ConditionalOp, Operations
...
def test_operation_run():
    ...
    with opBuilder as op:
        ...
        with op.loop(
            Operation(func=IterativeOp(MaxIterations(max_iters=4)), step_name="loop_increment", step_number=3)
        ) as loop:
            loop.step(Operation(func=SequentialOp(increment), step_name="looped_increment_1", step_number=4))
            loop.step(Operation(func=SequentialOp(increment), step_name="looped_increment_2", step_number=5))
