...
// main needs to be defined as amrex needs to be initialized
...
{
    {
        ...
        // Define MuliFab
        amrex::MultiFab mf(
            bArray, dm, ncomp, ngrow, amrex::MFInfo().SetArena(amrex::The_Pinned_Arena())
        );
        ...
        //  amrex::GpuArray<amrex::Real,3> dx = geom.CellSizeArray();  //dx[0] = dx dx[1] = dy dx[2]
        //  = dz
                ...
                for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi)
                {
                    ...
                    amrex::LoopOnCpu(
                        ...
                        [&](int i, int j, int k)
                        {
                            ...
                            amrex::Real r_squared = ((x - 0.5) * (x - 0.5) + (y - 0.5) * (y - 0.5)
                                                     + (z - 0.5) * (z - 0.5))
                                                  / 0.01;
