...
* **A conservative flux.** ``U = (0, 2x + 1.5z, 0)`` has no wall-normal component
  ...
  matters: OpenFOAM's ``kEpsilon``/``kOmegaSST`` carry dilatation terms
  (``fvm::SuSp((2/3) divU, k)``, ``kEpsilon.C:280``) that the NeoN closures drop
  as identically zero for divergence-free flow — the assumption every solver run
  satisfies, since the flux the closure gets is the one the pressure solve made
  conservative. Seeding a ``U`` whose interpolated flux is *not* conservative
  would measure that omission instead of the wall functions.
