...
"""Parity of scalar convection div(phi,T) under every div scheme.

pybFoam side uses the Gauss identity ``fvc.div(fvc.flux(phi, T, key="div(phi,T)"))``
(no scalar convection overload of ``fvc.div``); neon side is ``nn.exp.div(phi, T)``.
"""
