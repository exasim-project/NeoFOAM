...
"""Stage-1 contract: the PatchManifest schema (hermetic -- no FreeCAD/OpenFOAM)."""
...
def _manifest() -> PatchManifest:
    return PatchManifest(
        ...
        patches=[
            ...
            PatchEntry(name="outlet", stl="constant/triSurface/outlet.stl", role=PatchRole.outlet),
            ...
            PatchEntry(name="frontBack", stl="constant/triSurface/frontBack.stl", role=PatchRole.empty),
        ],
    )


def test_roundtrip_save_load(tmp_path: Path) -> None:
    """save() then load() reproduces the manifest exactly."""
    manifest = _manifest()
    path = manifest.save(tmp_path / "manifest.json")
    assert PatchManifest.load(path) == manifest
...
def test_fixture_validates_and_matches_tube_bank() -> None:
    ...
    manifest = PatchManifest.load(FIXTURE)
    assert manifest.geometry_source == "tube_bank.FCStd"
    assert manifest.scale_to_meters == 0.001
    assert manifest.bbox.max == (0.576, 0.16, 0.02)
...
def test_by_role_groups_both_walls() -> None:
    ...
    manifest = PatchManifest.load(FIXTURE)
    assert [p.name for p in manifest.by_role(PatchRole.wall)] == ["walls", "tubes"]
    assert [p.name for p in manifest.by_role(PatchRole.inlet)] == ["inlet"]
    assert [p.name for p in manifest.by_role(PatchRole.empty)] == ["frontBack"]
