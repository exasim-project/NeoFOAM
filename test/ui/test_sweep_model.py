...
def test_add_mesh_source_creates_and_extends_keyed_mesh_dimension() -> None:
    ...
    assert m.to_dimensions()["mesh"] == {"base": {"block_mesh_dict_config": {"scale": 1.0}}}
...
def test_remove_mesh_source_drops_dimension_when_last() -> None:
    ...
    m.add_mesh_source("snappy_hex_mesh_dict_config", title="s", schema=_SNAPPY_SCHEMA, seed={})
...
def test_overview_cad_times_config_case_count() -> None:
    m = SweepModel()
    m.add_cad_dimension("cad", title="CAD geometry", model_path="m", params={"tube_d": 6.0})
