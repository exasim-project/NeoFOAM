...
def compare_fields_numerically(
    ...
    Returns:
        Tuple of (match, max_abs_diff, max_rel_diff)
    """
    try:
        ...
        if np.allclose(internal1, internal2, rtol=rtol, atol=atol):
            ...
            return True, float(max_abs_diff), float(rel_diff)
        else:
            print(
                f"  {field_name}: ✗ Max abs diff: {max_abs_diff:.2e}, Max rel diff: {rel_diff:.2e}"
            )
            print(f"    Tolerance: rtol={rtol}, atol={atol}")
            return False, float(max_abs_diff), float(rel_diff)
...
def compare_solver_fields(
    ...
    Returns:
        Tuple of:
            ...
            - failed_details: dict[field_name, (max_abs_diff, max_rel_diff)]
    """
