...
``PressureVelocityAlgorithmNeoN.create`` is the case-free entry point (the
configuration names the algorithm); ``detect_and_create`` is the on-disk one and
needs a case, so only ``create`` is covered here. It hands back the spec module
singleton and stamps ``algorithm_type`` on it, which is what the solver reads
back later — so both are asserted.
"""
...
def test_create_rejects_an_unsupported_algorithm(algorithm_type: str) -> None:
    ...
    ``PISO`` is rejected here on purpose: it is a *case* shape, not a spec —
    ``detect_and_create`` maps a pisoFoam case onto the PIMPLE spec, which then
    reads the case's PISO control block.
    """
