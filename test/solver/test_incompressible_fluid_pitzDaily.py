...
"""
Integration test for the IncompressibleFluid solver using the pitzDaily case.
...
@requires_openfoam
def test_incompressible_fluid_pitzDaily():
    """Test the IncompressibleFluid solver on the pitzDaily tutorial case."""
    ...
    test_case = repo_root / "test_cases" / "pitzDaily_incompressible_test"
    ...
    try:
        ...
        # Replace endTime with a small value for testing
        control_dict_content = control_dict_content.replace(
            "endTime         0.3;",
            "endTime         0.001;"
        )
        # Set writeControl to timeStep to write every N time steps
        control_dict_content = control_dict_content.replace(
            "writeControl    adjustable;",
            "writeControl    timeStep;"
        )
        # Write every 2 time steps
        control_dict_content = control_dict_content.replace(
            "writeInterval   0.01;",
            "writeInterval   2;"
        )
        control_dict.write_text(control_dict_content)        # Change to the test case directory so OpenFOAM can find files
        ...
        time_dirs = sorted([d for d in test_case.iterdir() if d.is_dir() and d.name.replace(".", "").replace("-", "").isdigit()])
        ...
        # Check for turbulence model fields
        assert (final_time / "k").exists(), "Turbulent kinetic energy field k not found in final time"
        assert (final_time / "epsilon").exists(), "Turbulent dissipation field epsilon not found in final time"
