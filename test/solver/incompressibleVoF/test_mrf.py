...
**Case.** ``cases/vofRow4/common`` (four cells across a unit cube, ``0/U`` a
uniform ``(2 0 0)``, ``0/alpha.water`` stepping ``0, 0.25, 0.75, 1``) under the
``rotor`` overlay, which renames the block so blockMesh puts every cell in a
cellZone and adds a ``constant/MRFProperties`` spinning that zone at ``omega 2``
about ``(0 0 1)``.
...
def test_the_momentum_source_gains_the_mass_weighted_frame_acceleration(
    ...
    np.testing.assert_allclose(
        ...
        rtol=0,
        atol=_ATOL,
        err_msg="rotor: the momentum source does not carry -V*rho*(Omega x U)",
    )
