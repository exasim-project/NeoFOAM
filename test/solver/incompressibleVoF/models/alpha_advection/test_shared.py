...
def test_shared_steps_are_the_fields_with_their_edges_and_no_writes() -> None:
    ...
    # alone, alphaPhi10 (phi*interpolate(alpha1)) from phi and alpha1. VoF
    # writes through OpenFOAM's AUTO_WRITE, so none of them opts into
    ...
    assert [step.write for step in steps] == [False] * 8


def test_the_phase_fractions_and_densities_are_the_mixtures_own(
    vof_row4: BuiltCase,
) -> None:
    # alpha1/alpha2 are not read again — they are the mixture's fields, named
    # after the case's ``phases (water air)`` — and the two densities come from
...
def test_alpha_phi_un_is_registered_under_its_final_name_and_starts_at_zero(
    vof_row4: BuiltCase,
) -> None:
    # createAlphaFluxes.H: registered under the literal name "alphaPhiUn" so
    # e.g. the scalarTransport functionObject's ``phase`` lookup finds it, and
    # zero-initialised (dimensionedScalar(phi.dimensions(), Zero)); MULES fills
    # it in place during the alpha solve, not at build time.
