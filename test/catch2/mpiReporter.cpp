...
void MpiReporter::testRunEnded(const Catch::TestRunStats& stats)
{
    ...
    MPI_Reduce(&stats.aborting, &globalStats.aborting, 1, MPI_C_BOOL, MPI_LOR, ROOT, COMM);
    MPI_Reduce(
...
void MpiReporter::assertionEnded(const Catch::AssertionStats& stats)
{
    ...
    if (needPrint)
    {
        MPI_Send(&needPrint, 1, MPI_C_BOOL, ROOT, SERIALIZATION_TAG, COMM);

        bool allowedToPrint = false;
        MPI_Recv(&allowedToPrint, 1, MPI_C_BOOL, ROOT, SERIALIZATION_TAG, COMM, MPI_STATUS_IGNORE);
        ...
        MPI_Send(&finished, 1, MPI_C_BOOL, ROOT, SERIALIZATION_TAG, COMM);
    }
}
