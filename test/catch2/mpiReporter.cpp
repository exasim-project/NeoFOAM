...
void MpiReporter::assertionEnded(const Catch::AssertionStats& stats)
{
    ...
    if (needPrint)
    {
        MPI_Send(&needPrint, 1, MPI_CXX_BOOL, ROOT, 666, COMM);

        bool allowedToPrint = false;
        MPI_Recv(&allowedToPrint, 1, MPI_CXX_BOOL, ROOT, 666, COMM, MPI_STATUS_IGNORE);
        ...
        MPI_Send(&finished, 1, MPI_CXX_BOOL, ROOT, 666, COMM);
    }
}
