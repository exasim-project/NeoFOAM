...
class PressureAlgorithmSimple(BaseModel):
    ...
    _implementations = {
        False: StandardPressureImpl,
        True: BuoyantPressureImpl
    }
...
class PressureAlgorithmMulti(BaseModel):
    ...
    algorithm: str = AdaptableField(
        default="SIMPLE",
        description="Algorithm type: SIMPLE, PISO, or PIMPLE"
    )
    use_buoyancy: bool = AdaptableField(
        default=False,
        description="Use buoyancy terms"
    )
...
def test_adaptable_field_metadata():
    ...
    field_info = model.model_fields['use_buoyancy']
    ...
    tolerance_info = model.model_fields['tolerance']
...
def test_adaptable_field_with_complex_dispatch():
    ...
    for algo, buoyancy, expected_op in test_cases:
        ...
        ops = model.get_operations()
        assert expected_op in ops, f"Expected {expected_op} in operations for {algo}, buoyancy={buoyancy}"
