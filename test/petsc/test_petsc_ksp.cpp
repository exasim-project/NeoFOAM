...
int main(int argc, char** args)
{
    ...
    */
    ...
    { /* cannot directly set KSP/PC options when using the MPI linear solver server */
    ...
    */
    ...
    PetscCall(PetscPrintf(
        PETSC_COMM_SELF, "Norm of error %g, Iterations %" PetscInt_FMT "\n", (double)norm, its
    ));

    /* check that KSP automatically handles the fact that the new non-zero values in the matrix are
     * propagated to the KSP solver */
...
refine_always
   ...
   test:
      ...
      requires: cuda
      args: -pc_type sor -pc_sor_symmetric -ksp_monitor_short -ksp_gmres_cgs_refinement_type
refine_always -mat_type aijcusparse -vec_type cuda args: -ksp_view
   ...
   test:
      ...
      requires: cuda
      args: -pc_type eisenstat -ksp_monitor_short -ksp_gmres_cgs_refinement_type refine_always
-mat_type aijcusparse -vec_type cuda -ksp_view

   test:
      suffix: aijcusparse
      requires: cuda
      args: -ksp_monitor_short -ksp_gmres_cgs_refinement_type refine_always -mat_type aijcusparse
-vec_type cuda -ksp_view output_file: output/ex1_1_aijcusparse.out
...
-mpi_linear_solver_server_mat_view # the usual options for the linear solver (in this case using the
server) args: -ksp_monitor -ksp_converged_reason -mat_view -ksp_view -ksp_type cg -pc_type none #
the options for the prefixed objects args: -prefix_test_mpi_linear_solver_server_mat_view
-prefix_test_ksp_monitor -prefix_test_mpi_linear_solver_server_minimum_count_per_rank 5
   ...
   test:
      ...
      # use the MPI Linear Solver Server
      args: -mpi_linear_solver_server -mpi_linear_solver_server_view
-mpi_linear_solver_server_use_shared_memory false # controls for the use of PCMPI on a particular
system args: -mpi_linear_solver_server_minimum_count_per_rank 5 -mpi_linear_solver_server_ksp_view
-mpi_linear_solver_server_mat_view # the usual options for the linear solver (in this case using the
server) args: -ksp_monitor -ksp_converged_reason -mat_view -ksp_view -ksp_type cg -pc_type none #
the options for the prefixed objects args: -prefix_test_mpi_linear_solver_server_mat_view
-prefix_test_ksp_monitor -prefix_test_mpi_linear_solver_server_minimum_count_per_rank 5
