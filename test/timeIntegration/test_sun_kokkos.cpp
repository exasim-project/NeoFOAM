...
int main(int argc, char* argv[])
{
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        Kokkos::parallel_for(
            "fill_A",
            Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<3>>({0, 0, 0}, {nblocks, rows, cols}),
            KOKKOS_LAMBDA(const sunindextype i, const sunindextype j, const sunindextype k) {
        ...
        // Print result
        if (fails) std::cout << "FAIL: SUNLinSol module failed " << fails << " tests\n\n";
        else
...
static int CompareTol(sunrealtype a, sunrealtype b, sunrealtype tol)
{
    ...
    sunrealtype norm = std::min(std::abs(a + b), std::numeric_limits<sunrealtype>::max());
    return diff >= std::max(10 * std::numeric_limits<sunrealtype>::epsilon(), tol * norm);
}
