...
#. Configure and build NeoFOAM with the ``develop`` preset.
#. Run your application with the kernel logger enabled:
```bash
./run_with_kokkos_tool.sh debug kernel-logger [--log filename] ./your_neofoam_application
```
**For profiling:**
#. Configure and build NeoFOAM with the ``profiling`` preset.
#. Run your application with the desired profiling tool:
```bash
